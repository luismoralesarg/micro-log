...
export function InsightsView({ data, tags, people, darkMode }) {
  ...
  const insights = getInsights(data, t);

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard label={t('insights.entries')} value={insights.totalEntries} darkMode={darkMode} />
        <StatCard label={t('insights.words')} value={insights.totalWords.toLocaleString()} darkMode={darkMode} />
        <StatCard label={t('insights.starred')} value={insights.highlights} darkMode={darkMode} />
        <StatCard label={t('insights.streak')} value={`${insights.currentStreak}d`} sub={`${t('insights.best')}: ${insights.streak}d`} darkMode={darkMode} />
      </div>

      <div className={`${bgCard} p-4 rounded border ${border}`}>
        <div className={`text-xs ${textMuted} font-mono uppercase tracking-wide mb-3`}>{t('insights.last30days')}</div>
      ...
      <div className={`${bgCard} p-4 rounded border ${border}`}>
        <div className={`text-xs ${textMuted} font-mono uppercase tracking-wide mb-3`}>{t('insights.byWeekday')}</div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard label={t('insights.dreams')} value={insights.daysWithDreams} darkMode={darkMode} />
        <StatCard label={t('insights.notes')} value={insights.notes} darkMode={darkMode} />
        <StatCard label={t('insights.ideas')} value={insights.ideas} sub={`${insights.ideasDone} ${t('insights.done')}`} darkMode={darkMode} />
        <StatCard label={t('insights.quotes')} value={insights.wisdom} darkMode={darkMode} />
      </div>
      ...
      {(tags.length > 0 || people.length > 0) && (
        <div className="grid sm:grid-cols-2 gap-4">
          {tags.length > 0 && (
            <div className={`${bgCard} p-4 rounded border ${border}`}>
              <div className={`text-xs ${textMuted} font-mono uppercase tracking-wide mb-2`}>{t('insights.topTags')}</div>
          ...
          {people.length > 0 && (
            <div className={`${bgCard} p-4 rounded border ${border}`}>
              <div className={`text-xs ${textMuted} font-mono uppercase tracking-wide mb-2`}>{t('insights.topPeople')}</div>
